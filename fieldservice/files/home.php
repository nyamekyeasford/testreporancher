...
<body class="bg-[#111827]">
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 h-[60px] bg-gray-600/10 backdrop-blur-md border-b border-gray-600 pt-2 pb-2 transition duration-300 ease-in-out">
        <ul class="mt-[-5px]">
            ...
            <li style="float: left;"><a href="draftpage.php">View Draft</a></li>
...
<table class="min-w-[90%] divide-y divide-gray-700">
            ...
            <tbody class="bg-gray-800 divide-y divide-gray-700">
                <?php
                try {
                    $query = "SELECT * FROM tb_images WHERE is_draft = 0";
                    ...
                    if ($result && $result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo "<td class='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100'><a href='edit.php?id={$row['id']}'>" . htmlspecialchars($row['customer_name']) . "</a></td>";
                            echo "<td class='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100'><a href='edit.php?id={$row['id']}'>" . htmlspecialchars($row['date']) . "</a></td>";
                            echo "<td class='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100'><a href='edit.php?id={$row['id']}'>" . htmlspecialchars($row['type_of_service']) . "</a></td>";
                            echo "<td class='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100'>" . htmlspecialchars($row['creator']) . "</td>";
